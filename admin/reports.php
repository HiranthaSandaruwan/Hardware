...
<fieldset style="margin-bottom:1rem;border:1px solid #dfe3e8;padding:.6rem .9rem;background:#fff;border-radius:.45rem;">
	...
	<table class="table mini"><tr><th>Role</th><th>Status</th><th>Count</th></tr>
	<?php while($row=$userSummary->fetch_assoc()): ?>
		<tr><td><?= $row['role'] ?></td><td><?= $row['status'] ?></td><td><?= $row['c'] ?></td></tr>
	<?php endwhile; ?>
	</table>
</fieldset>
<fieldset style="margin-bottom:1rem;border:1px solid #dfe3e8;padding:.6rem .9rem;background:#fff;border-radius:.45rem;">
	...
	<div class="flex" style="gap:1rem;">
		<div style="flex:1;min-width:15rem;">
			...
			<table class="table mini"><tr><th>Status</th><th>Count</th></tr>
		...
		<div style="flex:1;min-width:15rem;">
			...
			<table class="table mini"><tr><th>Category</th><th>Count</th></tr>
			<?php while ($row = $byCategory->fetch_assoc()): ?>
				<tr><td><?= $row['category'] ?></td><td><?= $row['c'] ?></td></tr>
			<?php endwhile; ?>
			</table>
		</div>
	</div>
...
<fieldset style="margin-bottom:1rem;border:1px solid #dfe3e8;padding:.6rem .9rem;background:#fff;border-radius:.45rem;">
	...
	<table class="table mini"><tr><th>Method</th><th>Status</th><th>Count</th><th>Total</th></tr>
	<?php while($row=$paySummary->fetch_assoc()): ?>
		<tr><td><?= $row['method'] ?></td><td><?= $row['status'] ?></td><td><?= $row['c'] ?></td><td><?= number_format($row['total'],2) ?></td></tr>
	<?php endwhile; ?>
	</table>
</fieldset>
<fieldset style="margin-bottom:1rem;border:1px solid #dfe3e8;padding:.6rem .9rem;background:#fff;border-radius:.45rem;">
