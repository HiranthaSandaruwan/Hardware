...
if (isset($_GET['approve'])) {
    $id = (int)$_GET['approve'];
    $mysqli->query("UPDATE requests SET admin_status='Approved', updated_at=NOW() WHERE request_id=$id AND admin_status='Pending'");
    header('Location: requests_pending.php');
    exit;
}
if (isset($_GET['reject'])) {
    $id = (int)$_GET['reject'];
    $mysqli->query("UPDATE requests SET admin_status='Rejected', updated_at=NOW() WHERE request_id=$id AND admin_status='Pending'");
    header('Location: requests_pending.php');
    exit;
}
