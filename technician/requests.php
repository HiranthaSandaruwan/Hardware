...
if (isset($_GET['start'])) {
  $id = (int)$_GET['start'];
  $mysqli->query("UPDATE requests SET status='In Progress', technician_id=$uid, updated_at=NOW() WHERE request_id=$id AND (technician_id IS NULL OR technician_id=$uid) AND status IN ('Approved','In Progress')");
  header('Location: requests.php');
  exit;
}
if (isset($_GET['complete'])) {
  $id = (int)$_GET['complete'];
  $mysqli->query("UPDATE requests SET status='Completed', updated_at=NOW() WHERE request_id=$id AND technician_id=$uid AND status='In Progress'");
  header('Location: requests.php');
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($time) {
    $stmt = $mysqli->prepare("UPDATE requests SET appointment_time=?, technician_id=?, updated_at=NOW() WHERE request_id=? AND (technician_id IS NULL OR technician_id=?)");
    $stmt->bind_param('siii', $time, $uid, $id, $uid);
    $stmt->execute();
  }
  header('Location: requests.php');
  exit;
...
<table class="table">
  <tr><th>ID</th><th>Device</th><th>Status</th><th>Appointment</th><th>Action</th></tr>
