...
if(isset($_GET['received'])){ $aid=(int)$_GET['received']; $row=$mysqli->query("SELECT request_id FROM appointments WHERE appointment_id=$aid AND technician_id=$tid")->fetch_assoc(); if($row){ $mysqli->query("UPDATE appointments SET device_received=1 WHERE appointment_id=$aid"); $mysqli->query("UPDATE requests SET state='Device Received', updated_at=NOW() WHERE request_id=".$row['request_id']); } header('Location: accepted_appointments.php'); exit; }
if(isset($_GET['noshow'])){ $aid=(int)$_GET['noshow']; $row=$mysqli->query("SELECT request_id FROM appointments WHERE appointment_id=$aid AND technician_id=$tid")->fetch_assoc(); if($row){ $mysqli->query("UPDATE appointments SET no_show=1 WHERE appointment_id=$aid"); $mysqli->query("UPDATE requests SET state='No-Show', updated_at=NOW() WHERE request_id=".$row['request_id']); } header('Location: accepted_appointments.php'); exit; }
...
if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['status_update'])){
  ...
  if($status!=='Pending' && (!$row || !$row['device_received'])){ $msg='Device must be received first'; }
  else {
    $stmt=$mysqli->prepare('INSERT INTO repair_updates(request_id,technician_id,status,note,created_at) VALUES(?,?,?,?,NOW())');
    $stmt->bind_param('iiss',$rid,$tid,$status,$note); if($stmt->execute()){
      $newState=$status==='Pending'?'Device Received':$status; if($status==='Pending' && !$row['device_received']){ /* fallback already handled above */ }
      $mysqli->query("UPDATE requests SET state='".$mysqli->real_escape_string($newState)."', updated_at=NOW() WHERE request_id=$rid");
      if(in_array($newState,['Completed','Cannot Fix','Returned'])){ header('Location: completed.php?just=1'); exit; }
      $msg='Status updated';
...
<table class="table"><tr><th>ID</th><th>Request</th><th>Slot</th><th>Device / Attendance</th><th>State</th><th>Status Update</th></tr>
