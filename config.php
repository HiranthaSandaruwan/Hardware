...
function require_login() {
    if (!is_logged_in()) { header('Location: '.$GLOBALS['BASE_URL'].'/auth/login.php'); exit; }
}
function require_role($role) {
    if (!is_logged_in() || current_user()['role'] !== $role) { header('Location: '.$GLOBALS['BASE_URL'].'/auth/login.php'); exit; }
}
?>
