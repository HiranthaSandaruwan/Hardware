...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($username===''||$password===''){$err='All fields required';}
  elseif(!in_array($role,['user','technician'])){$err='Invalid role';}
  else {
    ...
    if($stmt->num_rows>0){$err='Username taken';}
      ...
      if($stmt2->execute()){$ok='Registered. Await admin approval.';} else {$err='Insert failed';}
    }
  }
}
?>
